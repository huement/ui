...
class TokenChords {
  constructor(huiDetails, outputFile, inputFile) {
    ...
    this.output = outputFile ? outputFile : 'scss/hui/_tokens.scss'
  ...
  assembleTypeJSON() {
    ...
    _.forEach(this.params.chords, function (value, i) {
      ...
      let htags_lineHeights_px = {
        h1: getLineHeight(ms(5), value.line, value.scale).toFixed(2) + 'px',
        h2: getLineHeight(ms(4), value.line, value.scale).toFixed(2) + 'px',
        h3: getLineHeight(ms(3), value.line, value.scale).toFixed(2) + 'px',
        h4: getLineHeight(ms(2), value.line, value.scale).toFixed(2) + 'px',
        h5: getLineHeight(ms(1), value.line, value.scale).toFixed(2) + 'px',
        h6: getLineHeight(ms(0), value.line, value.scale).toFixed(2) + 'px',
      }
  ...
  assembleGBJSON() {
    ...
    _.forEach(this.params.chords, (value, i) => {
  ...
  tackOnConfigData() {
    ...
    let tokenVars = [
      ...
      textUI.scssVar('mq-responsive', this.dataFile.mqResponsive, false, false),
      textUI.scssVar(
        'base-font-size',
  ...
  assmebleChordTokens() {
    ...
    textUI.statusTxt('[SASS] Responsive-Rhythmic-Typography Object Created!')

...
class S3FileManager {
  ...
  constructor() {
    ...
    this.s3BucketName = process.env.AWS_BUCKETNAME || this.packData.s3BucketName
  ...
  async processFiles() {
    ...
    if (this.finalListOfFiles.length > 0) {
      textUI.taskTxt(`${this.finalListOfFiles.length} Files Ready to Upload!`)
      this.startS3Upload()
    }
  }
